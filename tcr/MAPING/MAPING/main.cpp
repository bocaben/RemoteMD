...
int PrintRoomMap(void) 
{
	...
	for (int i = -4; i <= 4; i++)   // print robot position in silver
	{
		for (int j = -4; j <= 4; j++)
		{
	...
	for (int i = -4; i <= 4; i++) // print robot destination in blue
	{
		for (int j = -4; j <= 4; j++)
		{
			SDL_SetRenderDrawColor(renderer, 0, 0, 225, 255);
...
bool Navdistans(int x, int y)
{
	...
	while (done != 1)
	{
		...
		if (NavigationMap[x+macroMapSize/2][y+macroMapSize / 2]!=1)
		{
			if (sawfirst == 1)
			{
				time2 = clock() / CLOCKS_PER_SEC;
				cout << "culculating 5 more sec" << endl;
				sawfirst = 0;
			}
			if (time1 - time2 > 5)
			{
...
void fill_BuffMap(int buffer_size) //creat a list of points ajaisent to the obstacles that the robot can go thru tham. THIS BUFFER IS NOT ADDED TO THE ROOM MAP!.
{
	for (int i = 0; i < macroMapSize; i++)
	{
		for (int j = 0; j < macroMapSize; j++)
...
bool fill_navigationMap(int x, int y)
{
	clear_BuffMap();
	fill_BuffMap(15);
...
void creatmap(RPlidarDriver * drv, Serial* SP)
{
	...
	while (fine != 1)
	{
		...
		do {
			...
			if (fill_navigationMap(nextpoint[0], nextpoint[1]) == 1)
			{
				tryagain = 0;
				std::cout << "finish";
			}
			...
			while (OK == 0)
			{
					...
					// if the robot is stoped: 1. add a new abstacle. 2. go back. 3. calculate a new path to the next point. 4. go to the next point.
				{
					...
					location[0] += dX;
					location[1] += dY;
					fill_navigationMap(nextpoint[0], nextpoint[1]);
					//PrintNavMap();
					path(nextpoint[0], nextpoint[1]);
					//PrintRoomMap();
					optimize_path();
...
bool show_next_pos_on_map(int x, int y)	
{
	...
	for (int i = -4; i <= 4; i++)   // //print the next position in purple
	{
		for (int j = -4; j <= 4; j++)
		{
